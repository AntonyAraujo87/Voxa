...
$builtExe = Join-Path $env:GITHUB_WORKSPACE 'src-tauri/target/release/voxa.exe'
...
function Check-Installed {
    ...
    if ((Get-FileHash -LiteralPath $installedExe).Hash -ne (Get-FileHash -LiteralPath $builtExe).Hash) {
        throw 'Executavel instalado diverge do build validado'
    }
    & node scripts/check-native.mjs $installedExe
...
- SHA-256 do executavel instalado igual ao build, manifest e loader conferidos.
